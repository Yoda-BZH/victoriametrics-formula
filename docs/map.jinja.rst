...
We will see a quick setup to configure the ``victoriametrics`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``victoriametrics`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``victoriametrics`` formulas under ``/srv/salt/victoriametrics/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/victoriametrics/parameters/dns:domain/
    ln -s dns:domain /srv/salt/victoriametrics/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/victoriametrics/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/victoriametrics/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``victoriametrics`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``victoriametrics`` formulas under ``/srv/salt/victoriametrics/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/victoriametrics/parameters/roles
...
We create a configuration for the role ``victoriametrics/server`` in ``/srv/salt/victoriametrics/parameters/roles/victoriametrics/server.yaml``:
...
We create another configuration for the role ``victoriametrics/client`` in ``/srv/salt/victoriametrics/parameters/roles/victoriametrics/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles victoriametrics/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles victoriametrics/client
...
.. code-block:: console
    ...
    mv /srv/salt/victoriametrics/parameters/dns:domain/  '/srv/salt/victoriametrics/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-victoriametrics-as-json:
      test.nop:
...
Here is an example based on `template-formula/victoriametrics/config/file.sls`_:
...
This ``sls`` file expose a ``victoriametrics`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ victoriametrics['winner'] }}
...
.. _template-formula/victoriametrics/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/victoriametrics/config/file.sls
